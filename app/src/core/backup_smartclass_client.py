...
class SmartclassClient:
    ...
    def search_video(self, condition: VideoSearchCondition) -> VideoSearchResult:
        ...
        value_node = result.get("Value") or result.get("value") or result
        ...
        if data_node is None:
            raise ValueError(f"找不到视频数据列表。返回的 Keys: {result.keys()}")

        data = data_node
        total_count = result.get("TotalCount") or len(data)
