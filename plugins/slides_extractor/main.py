...
def _process_segment_safe(
    ...
    try:
        ...
        def report_segment_progress(current: int, _: int):
            nonlocal last_reported
            increment = max(0, current - last_reported)
            last_reported = current
            if progress_callback and increment:
...
def extract_slides_endpoint(request: ExtractRequest):
