...
MODEL_SIZE = "large-v3"
REMOTE_MODEL_ID = "Systran/faster-whisper-large-v3"
...
global_status = PluginServiceStatus(DEVICE, compute_type=COMPUTE_TYPE)
...
def _validate_model_files(model_path: str) -> bool:
    """校验 Faster-Whisper 启动所需的本地模型文件。"""
    required_files = {
        "model.bin": 2000 * 1024 * 1024,
        "config.json": 1,
        "tokenizer.json": 1,
        "vocabulary.json": 1,
    }
    return all(
        ...
        and os.path.getsize(os.path.join(model_path, name)) >= min_size
        for name, min_size in required_files.items()
    )
    ...
    if _validate_model_files(MODEL_DIR_PATH):
        ...
        try:
            actual_model_path = Aria2ModelDownloader().download_snapshot(
                ...
                MODEL_DIR_PATH,
                create_download_progress_callback(global_status, _report_to_main, 5, 80),
            )
        ...
        if not _validate_model_files(actual_model_path):
            raise RuntimeError("Whisper 模型文件下载后校验失败")

    # ── 阶段2：加载模型到设备 ──
    print(f"--- 正在加载模型到设备: {DEVICE}, 计算类型: {COMPUTE_TYPE} ---")
    global_status.update(phase="loading", message=f"正在加载模型到 {DEVICE.upper()}...", progress=80, total_size=0, downloaded_size=0, speed=0)
    _report_to_main("loading", f"正在加载模型到 {DEVICE.upper()}...", progress=80, total_size=0, downloaded_size=0, speed=0)
    try:
        model = WhisperModel(
            actual_model_path,
            device=DEVICE,
...
task_manager = get_task_manager(
    storage_dir=os.path.join(BASE_DIR, "tasks"),
    max_workers=2,
    cleanup_task_files=True,
)
