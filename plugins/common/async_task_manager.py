...
class AsyncTaskManager:
    ...
    def __init__(self, storage_dir: str = None, max_workers: int = 2, cleanup_task_files: bool = False):
        """
        初始化任务管理器
        ...
        self.cleanup_task_files = cleanup_task_files
    ...
    def _cleanup_task_files(self, task: AsyncTask):
        """清理任务相关的临时文件"""
        if not self.cleanup_task_files:
            return
        try:
            # 清理结果文件
...
def get_task_manager(
    ...
    max_workers: int = 2,
    cleanup_task_files: bool = False,
) -> AsyncTaskManager:
    """获取全局任务管理器实例"""
    global _task_manager
    if _task_manager is None:
        _task_manager = AsyncTaskManager(storage_dir, max_workers, cleanup_task_files)
