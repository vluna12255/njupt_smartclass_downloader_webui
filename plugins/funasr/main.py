...
from common.service_status import PluginServiceStatus, create_startup_reporter
...
VAD_MODEL_DIR_PATH = os.path.join(BASE_DIR, "fsmn-vad")
PUNC_MODEL_DIR_PATH = os.path.join(BASE_DIR, "ct-punc")
...
VAD_REMOTE_MODEL_ID = "iic/speech_fsmn_vad_zh-cn-16k-common-pytorch"
PUNC_REMOTE_MODEL_ID = "iic/punc_ct-transformer_zh-cn-common-vocab272727-pytorch"
...
global_status = PluginServiceStatus(DEVICE)
_report_to_main = create_startup_reporter(MAIN_SERVER_URL, PLUGIN_NAME)


def _validate_model_directory(path):
    if not os.path.isdir(path):
        return False
    for root, _, files in os.walk(path):
        for name in files:
            target = os.path.join(root, name)
            # A config-only residue is not a usable model snapshot.
...
def _validate_model_files():
    return all(_validate_model_directory(path) for path in (
        MODEL_DIR_PATH,
        VAD_MODEL_DIR_PATH,
        PUNC_MODEL_DIR_PATH,
    ))
...
def check_and_download_models():
    global_status.update(phase="checking", message="正在检查 FunASR 模型文件...", progress=10, total_size=0, downloaded_size=0, speed=0)
    _report_to_main("checking", "正在检查 FunASR 模型文件...", progress=10, total_size=0, downloaded_size=0, speed=0)
    if _validate_model_files():
    ...
    try:
        ...
        snapshots = [
            ...
            (modelscope_model_files(PUNC_REMOTE_MODEL_ID), PUNC_MODEL_DIR_PATH),
        ]
        Aria2ModelDownloader().download_snapshots(
            snapshots,
            create_download_progress_callback(global_status, _report_to_main, 10, 60),
        )
    except Exception as download_error:
        message = f"FunASR 模型下载失败: {download_error}"
        global_status.update(phase="failed", message=message, error=message, progress=0, speed=0)
        _report_to_main("failed", message, progress=0, success=False, speed=0)
        raise RuntimeError(message) from download_error
    if not _validate_model_files():
        raise RuntimeError("FunASR 模型文件下载后校验失败")
    global_status.update(phase="checking", message="FunASR 模型文件检查完成", progress=60, total_size=0, downloaded_size=0, speed=0)
    _report_to_main("checking", "FunASR 模型文件检查完成", progress=60, total_size=0, downloaded_size=0, speed=0)

# 执行检查
global_status.update(phase="initializing", message="FunASR 服务启动中...", progress=5)
_report_to_main("initializing", "FunASR 服务启动中...", progress=5, total_size=0, downloaded_size=0, speed=0)
...
try:
    ...
    global_status.update(phase="loading", message=f"正在加载 FunASR 模型到 {device.upper()}，请稍候...", progress=65, total_size=0, downloaded_size=0, speed=0)
    _report_to_main("loading", f"正在加载 FunASR 模型到 {device.upper()}，请稍候...", progress=65, total_size=0, downloaded_size=0, speed=0)
    model = AutoModel(
        model=MODEL_DIR_PATH,
        trust_remote_code=True,
        vad_model=VAD_MODEL_DIR_PATH,
        vad_kwargs={"max_single_segment_time": 10000},
        punc_model=PUNC_MODEL_DIR_PATH,
        device=device,
        disable_update=True,
    )
    if model is None:
